...
void calc_all_dists(int i,int s,int e,efloatarray& dists,int n,int tn)
{
  int l,j,k;
...
{ 
  ...
  f.open(profile,"r");
  estr line;
  efloatarray avgcons,bpcons,gapcons;
  int i,j,k,l;
  while (f.readln(line)){
    if (line.len()==0 || line[0]=='#') continue;
    line.trim();
    estrarray parts=re_explode(line," +");
    
    ldieif(parts.size()<4,"not enough parts in line: "+line);
    bpcons.add(parts[1].f());
    avgcons.add(parts[2].f());
    gapcons.add(parts[3].f());
  }
  ...
  float avgbppos,bppos,gappos;
  for (j=0; j<arr.size(); ++j){
    efloatarray fulldist;
    fulldist.init(arr.size());
    for (i=0; i<ncpus; ++i)
      taskman.addTask(calc_all_dists,evararray((const int&)j,0,seqlen,fulldist,(const int&)i,(const int&)ncpus));
    taskman.wait();

    efloatarray segmentdist;
    segmentdist.init(arr.size());
    for (i=0; i<ncpus; ++i)
      taskman.addTask(calc_all_dists,evararray((const int&)j,0,winlen,segmentdist,(const int&)i,(const int&)ncpus));
    taskman.wait();
    eintarray si(iheapsort(segmentdist));
    eintarray si2(iheapsort(fulldist));
    float tmpdist=segmentdist[si[si.size()-1]];
    for (l=si.size()-1; (l>=si.size()-10 || tmpdist==segmentdist[si[l]]) && l>=0; --l){ 
      bppos=accCons(bpcons,0,10);
      avgbppos=accCons(avgcons,0,10);
      gappos=accCons(gapcons,0,10);
      for (k=20; k<seqlen; k+=10){
        bppos+=accCons(bpcons,k-10,k);
        avgbppos+=accCons(avgcons,k-10,k);
        gappos+=accCons(gapcons,k-10,k);
        cout << j << " " << k << " " << bppos << " " << avgbppos << " " << gappos << " " << si[l] << " " << dist_short_compressed(arr[j],arr[si[l]],0,k) << " " << fulldist[si[l]] << " " << arr.keys(j) << " " << arr.keys(si[l]) << endl;
      }
      cout << endl;
    }
    for (l=si2.size()-1; fulldist[si2[l]]>=0.98 && l>=0; --l){ 
      if (segmentdist[si2[l]]==segmentdist[si2.size()-1]) continue; // already drawn before
      bppos=accCons(bpcons,0,10);
      avgbppos=accCons(avgcons,0,10);
      gappos=accCons(gapcons,0,10);
      for (k=20; k<seqlen; k+=10){
        ...
        avgbppos+=accCons(avgcons,k-10,k);
        gappos+=accCons(gapcons,k-10,k);
        cout << j << " " << k << " " << bppos << " " << avgbppos << " " << gappos << " " << si2[l] << " " << dist_short_compressed(arr[j],arr[si2[l]],0,k) << " " << fulldist[si2[l]] << " " << arr.keys(j) << " " << arr.keys(si2[l]) << endl;
      }
      cout << endl;
    }
    cout << endl;
